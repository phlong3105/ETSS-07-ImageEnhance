...
def test(args, test_img_loader, model, model_fn_test, save_path, compute_metrics):
    tbar         = tqdm(test_img_loader)
    total_psnr   = 0
    total_ssim   = 0
    total_lpips  = 0
    total_time   = 0
...
def main():
    ...
    args.BATCH_SIZE = 1
    test_img_loader = create_dataset(args, data_path=test_path, mode='test')

    # test
    test(args, test_img_loader, model, model_fn_test, save_path, compute_metrics)
