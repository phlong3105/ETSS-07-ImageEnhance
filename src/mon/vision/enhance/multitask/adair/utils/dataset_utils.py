...
class AdaIRTrainDataset(Dataset):
    ...
    def _crop_patch(self, img_1: torch.Tensor, img_2: torch.Tensor) -> torch.Tensor:
        h       = img_1.shape[0]
        w       = img_1.shape[1]
    ...
    def _get_gt_name(self, rainy_name: str) -> str:
        gt_name = rainy_name.split("rainy")[0] + "gt/norain-" + rainy_name.split("rain-")[-1]
        return gt_name
    
    def _get_deblur_name(self, deblur_name: str) -> str:
        gt_name = deblur_name.replace("blur", "sharp")
        return gt_name
    
    def _get_enhance_name(self, enhance_name: str) -> str:
        gt_name = enhance_name.replace("low", "gt")
        return gt_name
    
    def _get_nonhazy_name(self, hazy_name: str) -> str:
...
class DenoiseTestDataset(Dataset):
    ...
    def tile_degrad(self, input_, tile=128, tile_overlap=0):
