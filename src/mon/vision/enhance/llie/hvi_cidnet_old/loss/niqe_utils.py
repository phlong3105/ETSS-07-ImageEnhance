...
def _convert_input_type_range(img):
    ...
    if img_type == np.float32:
        ...
        raise TypeError(f'The img type should be np.float32 or np.uint8, got {img_type}')
    return img
...
def _convert_output_type_range(img, dst_type):
    ...
    if dst_type not in (np.uint8, np.float32):
        raise TypeError(f'The dst_type should be np.float32 or np.uint8, got {dst_type}')
